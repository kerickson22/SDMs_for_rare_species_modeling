...
for (i in 1:length(results$species)) {
  if (file.exists(paste0(path2, "/models/", modelType[1], "/", results$species[i], "/", results$size[i], "/", "model_", results$rep[i], ".RData"))) {
    ...
    results$RMSEWeighted[i] <- model$RMSEWeighted
    results$TjursR2[i] <- model$TjursR2
    if (names(model)[11] =="P1_rankCor")  {results$PC1_rankCor[i] <- model$P1_rankCor} #fix typo from original
    if (names(model)[11] =="PC1_rankCor") {results$PC1_rankCor[i] <- model$PC1_rankCor}
    results$PC3_rankCor[i] <- model$PC3_rankCor
    results$pr_integral[i] <- model$pr_integral
    results$varPC2Response[i] <- model$varPC2Response
  }
...
results_ESM_simple$model <- rep(models[3], length(results_glm$species))
rm(results)
...
results_ESM_complex$model <- rep(models[4], length(results_glm$species))
rm(results)
...
results_Hmsc_joint$model <- rep(models[5], length(results_glm$species))
rm(results)
...
results_SAM <- results
results_SAM$model <- rep(models[6], length(results_glm$species))
rm(results)
...
#RECODE SIZES #####
full$size <- full$size %>% recode("size4" = "4", "size8"="8", "size16"="16", "size32"="32",
                                  "size64"="64") %>%
...
for (i in 1:length(full$model)) {
  if(full$model[i] == "glm") {
    full$pos[i] = as.numeric(full$size[i]) - 0.4
  }
  if(full$model[i] == "ESM_simple") {
    full$pos[i] = as.numeric(full$size[i]) - 0.2
  }
...
for (i in 1:length(converged$model)) {
  if(converged$model[i] == "glm") {
    converged$pos[i] = as.numeric(converged$size[i]) - 0.4
  }
  if(converged$model[i] == "ESM_simple") {
    converged$pos[i] = as.numeric(converged$size[i]) - 0.2
  }
